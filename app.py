...
RSS_FEEDS = {
    ...
    "India": {
        "The Hindu": {"url": "[https://news.google.com/rss/search?q=site:thehindu.com+when:1d&hl=en-IN&gl=IN&ceid=IN:en](https://news.google.com/rss/search?q=site:thehindu.com+when:1d&hl=en-IN&gl=IN&ceid=IN:en)", "bias": "Left"},
        "NDTV": {"url": "[https://feeds.feedburner.com/ndtvnews-india-news](https://feeds.feedburner.com/ndtvnews-india-news)", "bias": "Center"},
        "Times of India": {"url": "[https://news.google.com/rss/search?q=site:timesofindia.indiatimes.com+india+when:1d&hl=en-IN&gl=IN&ceid=IN:en](https://news.google.com/rss/search?q=site:timesofindia.indiatimes.com+india+when:1d&hl=en-IN&gl=IN&ceid=IN:en)", "bias": "Right"},
    },
    "Tamil Nadu": {
        "The Hindu TN": {"url": "[https://news.google.com/rss/search?q=site:thehindu.com+tamil+nadu+when:1d&hl=en-IN&gl=IN&ceid=IN:en](https://news.google.com/rss/search?q=site:thehindu.com+tamil+nadu+when:1d&hl=en-IN&gl=IN&ceid=IN:en)", "bias": "Left"},
        "Indian Express TN": {"url": "[https://news.google.com/rss/search?q=site:indianexpress.com+chennai+when:1d&hl=en-IN&gl=IN&ceid=IN:en](https://news.google.com/rss/search?q=site:indianexpress.com+chennai+when:1d&hl=en-IN&gl=IN&ceid=IN:en)", "bias": "Center"},
        "TOI Chennai": {"url": "[https://news.google.com/rss/search?q=site:timesofindia.indiatimes.com+chennai+when:1d&hl=en-IN&gl=IN&ceid=IN:en](https://news.google.com/rss/search?q=site:timesofindia.indiatimes.com+chennai+when:1d&hl=en-IN&gl=IN&ceid=IN:en)", "bias": "Right"},
    },
}
...
def _safe_parse_json(raw: str) -> dict | list:
    raw = raw.strip()
    # Build backticks dynamically so copy-paste doesn't break
    marker = chr(96) * 3
    pattern_start = r"^" + marker + r"(?:json)?\s*"
    pattern_end = marker + r"\s*$"
    
    raw = re.sub(pattern_start, "", raw, flags=re.MULTILINE)
    raw = re.sub(pattern_end, "", raw, flags=re.MULTILINE)
    try:
...
def generate_all_metadata(clusters: list[list[dict]]) -> list[dict]:
    ...
    json_schema = """[
  {
    ...
    try:
        ...
        return [_single_cluster_metadata([a["title"] for a in c]) for c in clusters]

def _single_cluster_metadata(titles: list) -> dict:
    json_schema = """{
    "summary": "Factual 2-sentence summary.",
    "insights": {"key_fact": "One verified fact.", "discrepancy": "Media differences or 'Single source narrative'."},
    "topics": ["Tag1", "Tag2"]
}"""
    prompt = (
        f"Analyse these headlines:\n" + "\n".join(titles) + "\n\n"
        "Return JSON strictly following this structure. topics: 1–3 broad categories:\n" + json_schema
    )
    try:
        raw = _call_gemini_with_fallback(prompt)
...
def process_live_news(region: str) -> list[dict]:
    ...
    for cluster, ai_data in zip(clusters, all_metadata):
        if not isinstance(ai_data, dict) or "error" in ai_data: continue
...
def render_event_card(event: dict) -> None:
    ...
    topics_html = "".join(
        f"<span style='display:inline-block;background:#f3f4f6;color:#374151;padding:4px 10px;border-radius:16px;font-size:12px;margin-right:6px;font-weight:500;'>{t}</span>"
        for t in event.get("topics", [])
    )

    sources_html = ""
    for art in sources:
        color = "#3b82f6" if art.get("bias") == "Left" else "#eab308" if art.get("bias") == "Center" else "#ef4444"
        sources_html += f"<div style='margin-bottom:8px;'><small><b><span style='color:{color}'>▪ {art.get('bias','?')}</span> | {art.get('source','?')}</b>: <a href='{art.get('link','#')}' style='color:#333;text-decoration:none;'>{art.get('title','')}</a></small></div>"
...
"""
        ...
        bias_color = "#3b82f6" if bias == "Left" else "#eab308" if bias == "Center" else "#ef4444"
        bias_ui = f"<p style='font-size:13px;color:{bias_color};font-weight:bold;margin:16px 0;'>▪ Single Source Narrative ({bias} Leaning)</p>"
...
def build_view(region: str) -> None:
    ...
    with st.spinner(f"Loading {region} news…"):
        events = process_live_news(region) if "Live" in view_mode else fetch_weekly_radar(region)
    ...
    shown = 0
    for event in events:
        if selected_topics and not any(t in selected_topics for t in event.get("topics", [])):
            continue
